...
class TestPartnerRepetition8Players(unittest.TestCase):
    def setUp(self):
    ...
    def test_partner_repetition_limit_8_players(self):
        # Game 1: P0 + P1
        team1 = ["p0", "p1"]
        team2 = ["p2", "p3"]
        ...
        # Game 2: P0 + P4 (P0 plays again with someone else to clear "back-to-back" personal check)
        team3 = ["p0", "p4"]
        team4 = ["p5", "p6"]
        ...
        # P0's last game with P1 was Game 1.
        # Elapsed = 2 - 1 = 1.
        
        # Case 1: Gap = 1. Expected FAIL (Constraint 3)
        self.assertFalse(can_play_with_player(self.session, "p0", "p1", "partner"),
                         "Should not allow partner with gap 1")

        # Game 3: Random others
        match3 = Match(id="m3", court_number=1, team1=["p2","p3"], team2=["p5","p6"], status="completed")
        self.session.matches.append(match3)
        # Global Games: 3. Elapsed = 3 - 1 = 2.
        
        # Case 2: Gap = 2. Expected FAIL (Constraint 3)
        self.assertFalse(can_play_with_player(self.session, "p0", "p1", "partner"),
                         "Should not allow partner with gap 2")

        # Game 4: Random others
        match4 = Match(id="m4", court_number=2, team1=["p2","p4"], team2=["p3","p7"], status="completed")
        self.session.matches.append(match4)
        # Global Games: 4. Elapsed = 4 - 1 = 3.
        
        # Case 3: Gap = 3. Expected PASS (Constraint 3)
        self.assertTrue(can_play_with_player(self.session, "p0", "p1", "partner"),
                        "Should allow partner with gap 3")
